...
import { ExternalLink, Clock, Code2, Image as ImageIcon, Video, MessageSquare, ChevronDown, ChevronUp, Eye, Trash2, Edit3, Share2, Layers, Sparkles, Loader2, Link2, Bot, Highlighter } from 'lucide-react';
...
export default function AICardNode({ id, data }: { id: string; data: any }) {
  ...
  const applyHighlight = async () => {
    ...
    // Find the exact text and wrap it in the highlight syntax
    const newContent = data.content.replace(selectedText, `==${selectedText}==`);
    await updateDoc(doc(db, 'snippets', id), { 'data.content': newContent });
    setShowHighlightBtn(false);
    setSelectedText('');
  ...
  return (
    <div ref={cardRef} className={`w-[400px] max-w-[90vw] bg-white border ${isHighlighted ? 'border-purple-500 ring-4 ring-purple-500/30 shadow-2xl scale-105' : 'border-gray-200 shadow-sm'} rounded-2xl transition-all duration-300 font-sans group relative flex flex-col`}>
      ...
      {showHighlightBtn && (
        <div className="absolute -top-12 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-2 fade-in">
          <button onClick={applyHighlight} className="bg-gray-900 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-xl flex items-center gap-2 hover:bg-black transition-colors">
            <Highlighter className="w-4 h-4 text-yellow-400" /> Highlight Text
          </button>
        </div>
      )}
      ...
      <div 
        ...
        className={`p-5 nodrag cursor-text text-sm text-gray-800 bg-white relative ${!isExpanded && (data.type === 'text' || data.type === 'code') ? 'max-h-[280px] overflow-hidden' : ''}`}
      >
        {data.type === 'bookmark' ? (
